...
public:
    ...
    Array<Array<Array <double> > > AC;                       ///< Array of activation inputs
...
inline void Domain::Forward (size_t Nproc)
{
    ...
    for (size_t nt=0;nt<NT;nt++)
    {
        for (size_t nh=0;nh<=NH;nh++)
        {
            if (nh==0)
            {
                ParMult(W[nh],I[nt],AC[nt][nh],1);             
            }
            else
            {
                ParMult(W[nh],OT[nt][nh-1],AC[nt][nh],1);             
            }
            for (size_t nn=0;nn<N[nh];nn++)
            {
                // if (type != 1)
...
inline void Domain::Predict (Array<double> & Input, Array<double> & Predict, size_t Nproc)
{
    for (size_t nh=0;nh<=NH;nh++)
    {
        if (nh==0)
        {
            ParMult(W[nh],Input,AC[0][nh],Nproc);             
        }
        else
        {
            ParMult(W[nh],OT[0][nh-1],AC[0][nh],Nproc);
        }
        for (size_t nn=0;nn<N[nh];nn++)
        {
        //     if (type != 1)
