	...
	for (size_t i=0; i<Particles.Size(); i++)
    {
        for (size_t j=0;j<DEMParticles.Size();j++)
        {
            ...
            if (Particles[i]->Material == 3 || Particles[i]->Material == 2) //Interaccion between solid particles and DEM particles
            {
                ...
                {
                    Vec3_t n = (xf-xi)/dist;
                    Vec3_t vrel = Particles[i]->v - DEMParticles[j]->v;
                    Vec3_t vt = vrel-dot(vrel,n)*n;
                    Vec3_t Fn = DEMParticles[j]->Props.Kn*delta*n;
                    ...
                    Particles[i]->a += Fn/Particles[i]->Mass - DEMParticles[j]->Props.Kn*FrictionMap[j][i]/Particles[i]->Mass;
                    omp_set_lock  (&DEMParticles[j]->lck);
                    DEMParticles[j]->F -= Fn - DEMParticles[j]->Props.Kn*FrictionMap[j][i];
            ...
            else //Interaction for fluid particles with DEM particles
            {
                ...
                if (dist < Cellfac*h)
                {
                    ...
                    if (delta > 0.0) Fn = DEMParticles[j]->Props.Kn*delta*n;
                    Vec3_t vrel = Particles[i]->v - DEMParticles[j]->v;
		            double GK	= 2.0/(3.0*h*pow(h,Dimension-1))*GradKernel(Dimension, KernelType, dist/h, h)/Kernel(Dimension, KernelType, 0.0, h);
		            //double GK	= GradKernel(Dimension, KernelType, dist/h, h);
		            Vec3_t VI = 0.0;
		            Viscous_Force(VisEq, VI, Particles[i]->Mu, Particles[i]->Density, Particles[i]->Density, GK, vrel, Dimension, KernelType, dist, h, xij, vrel);
                    Particles[i]->a += Fn/mass + mass*VI;
                    ...
                    omp_set_lock  (&DEMParticles[j]->lck);
                    DEMParticles[j]->F -= Fn + mass*mass*VI;
                    omp_unset_lock  (&DEMParticles[j]->lck);
                }
            }
        }
